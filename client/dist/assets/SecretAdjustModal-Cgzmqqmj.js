import{r as l,j as e,a0 as _e,a1 as Ve,aS as qe,A as Re,m as $e,H as Ge,s as se,a6 as He,a7 as Ue,k as Je,b as Ye,R as ke,aO as Ke,b3 as We,af as Le,b5 as Qe,J as Ie,x as Te,b6 as Xe,b7 as Ze,aV as et,ag as tt,b8 as at,a3 as ot,b9 as Fe,ba as rt,bb as it,aa as nt,b2 as st,C as Me,q as lt,bc as dt,bd as ct,be as pt}from"./vendor-BVroOj2J.js";import{g as ge,s as he,f as Pe,u as De,a as mt}from"./index-sEpZnVGC.js";import{r as xt}from"./searchEngine-BMYcElFi.js";import{d as ft,S as O,h as gt,i as ht,j as bt,M as be,a as je,B as R,f as J,I as Ae}from"./POS.styles-SnKkPsfK.js";import"./AlertModal--TwzGPyw.js";const w=o=>{const f=Number(o);return isNaN(f)||!isFinite(f)?0:f};function ut(o){if(!Array.isArray(o)||o.length===0)return[];const f=new Set,a=[];for(const b of o)b.id&&f.has(b.id)||(b.id&&f.add(b.id),a.push(b));return a}const jt=(o,f=0,a=36.6)=>{const b=Math.max(0,w(f)),S=w(a)||36.6,p={ventasContado:[],devoluciones:[],cancelaciones:[],entradas:[],salidas:[],abonos:[],ajustes:[]};let n=0,i=0,u=0,c=0,I=0,T=0,F=0,m=0;const E=ut(Array.isArray(o)?o:[]);let t=0,x=0,j=0,v=0,K=0,$=0,r=0,W=0,V=0;for(const M of E){let g=((M==null?void 0:M.type)||"").toLowerCase().trim();g=g.normalize("NFD").replace(/[\u0300-\u036f]/g,"");let C=(M==null?void 0:M.pagoDetalles)||{};if(typeof C=="string")try{C=JSON.parse(C)}catch{C={}}(!C||typeof C!="object")&&(C={});const D=w(M.amount),le=w(C.totalVenta)||D,A=w(C.efectivo),q=w(C.dolares),ee=w(C.cambio),Q=w(C.ingresoCaja),L=w(C.tarjeta),y=w(C.transferencia),Y=w(C.credito);let P=le||D;(g==="salida"||g.includes("devolucion")||g.includes("cancelacion")||g.includes("anulacion"))&&(P=-Math.abs(P));const B={...M,pagoDetalles:C,displayAmount:P};if(g.startsWith("venta")||g.includes("abono")||g.includes("pedido")||g.includes("apartado")?(u+=L,c+=y,I+=Y):g==="ajuste"&&(C.target==="tarjeta"&&(u+=D),C.target==="credito"&&(I+=D),C.target==="transferencia"&&(c+=D)),g.startsWith("venta")){if(A>.001||q>.001||ee>.001)n+=A-ee,i+=q,t+=A-ee,x+=q;else if(Q>.001)n+=Q,t+=Q;else{const G=L+y+Y,z=le-G;z>.001&&(n+=z,t+=z)}j+=L,v+=y,K+=Y}else if(g.includes("abono")||g.includes("liquidación")||g.includes("liquidacion")||g.includes("pedido")){if(q>.001)i+=q,n+=A,r+=q,$+=A;else if(A>.001)n+=A,$+=A;else if(Q>.001)n+=Q,$+=Q;else{const G=L+y,z=Math.max(0,D-G);n+=z,$+=z}W+=L,V+=y}else if(g==="entrada")n+=Math.abs(D);else if(g==="salida")n-=Math.abs(D);else if(g.includes("devolucion")||g.includes("cancelacion")||g.includes("anulacion"))if(C.ingresoCaja!==void 0&&C.ingresoCaja!==null)n+=w(C.ingresoCaja);else if(A>.001)n-=A;else{const G=L+y+Y,z=Math.abs(D)-G;z>.001&&(n-=z)}else if(g==="ajuste")C.target==="efectivo"?(n+=D,C.hidden&&(m+=D)):C.target==="dolares"&&(i+=D);else{const G=L+y+Y,z=D-G;Math.abs(z)>.001&&(n+=z)}g.startsWith("venta")||g.includes("abono")||g.includes("liquid")||g.includes("pedido")?F+=Math.abs(P):g.includes("devolucion")||g.includes("cancelacion")||g.includes("anulacion")?F-=Math.abs(P):(g==="ajuste"&&C.target==="ventas_totales"||g==="ajuste"&&C.target==="efectivo"&&D<0)&&(F+=D),g.startsWith("venta")?p.ventasContado.push(B):g.includes("devolucion")?(p.devoluciones.push(B),T+=Math.abs(P)):g.includes("cancelacion")||g.includes("anulacion")?(p.cancelaciones.push(B),T+=Math.abs(P)):g==="entrada"?p.entradas.push(B):g==="salida"?p.salidas.push(B):g.includes("abono")?p.abonos.push(B):g==="ajuste"&&p.ajustes.push(B)}n=w(n),i=w(i),u=w(u),c=w(c),I=w(I),F=w(F),m=w(m),T=w(T);const Z=b+n,k=Z+i*S;return{cajaInicial:b,netCordobas:n,netDolares:i,efectivoEsperado:w(k),efectivoEsperadoCordobas:w(Z),efectivoEsperadoDolares:i,totalVentasDia:w(F),totalTarjeta:u,totalTransferencia:c,totalCredito:I,totalNoEfectivo:w(u+c+I),sumDevolucionesCancelaciones:T,totalHidden:m,tasaRef:S,lists:p,vEfectivoC:w(t),vEfectivoD:w(x),vTarjeta:w(j),vTransf:w(v),vCredito:w(K),aEfectivoC:w($),aEfectivoD:w(r),aTarjeta:w(W),aTransf:w(V)}},ve=o=>Number(o||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});async function Be(o,f=4){const a=localStorage.getItem("token"),b=o.filter(n=>{const i=ge(n);return!i||i!=="loading"&&i!=="none"});let S=0;async function p(){if(S>=b.length)return;const n=b[S++];if(ge(n))return p();he(n,"loading");try{const i=await Pe(n,a),u=(i==null?void 0:i.imagen)||null;he(n,u||"none")}catch{he(n,"none")}return p()}await Promise.all(Array.from({length:f},p))}function yt(o){const[f,a]=l.useState(()=>{const p=ge(o);return p&&p!=="loading"&&p!=="none"?p:null}),b=l.useRef(null),S=l.useCallback(async()=>{if(!o)return;const p=ge(o);if(!(p==="loading"||p==="none")){if(p&&p!=="loading"){a(p);return}he(o,"loading");try{const n=localStorage.getItem("token"),i=await Pe(o,n),u=(i==null?void 0:i.imagen)||null;he(o,u||"none"),a(u||null)}catch{he(o,"none"),a(null)}}},[o]);return l.useEffect(()=>{const p=ge(o);if(p&&p!=="loading"){a(p!=="none"?p:null);return}const n=setInterval(()=>{const u=ge(o);u&&u!=="loading"&&(a(u!=="none"?u:null),clearInterval(n))},200),i=new IntersectionObserver(u=>{u[0].isIntersecting&&S()},{rootMargin:"200px"});return b.current&&i.observe(b.current),()=>{i.disconnect(),clearInterval(n)}},[o,S]),{imgSrc:f,cardRef:b}}function vt({productId:o,productName:f,onView:a}){const{imgSrc:b,cardRef:S}=yt(o);return e.jsxs("div",{ref:S,className:"image-placeholder",style:{position:"relative",height:160,background:"#f8fafc",display:"flex",alignItems:"center",justifyContent:"center",borderBottom:"1px solid #f1f5f9",overflow:"hidden"},children:[b&&e.jsx("div",{className:"eye-icon",onClick:p=>{p.stopPropagation(),a(b)},style:{position:"absolute",top:10,left:10,zIndex:20,background:"white",borderRadius:"50%",width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 6px rgba(0,0,0,0.1)",cursor:"pointer",transition:"transform 0.2s"},title:"Ver imagen",children:e.jsx(He,{size:14,color:"#64748b"})}),b?e.jsx("img",{src:b,alt:f,loading:"lazy",style:{maxWidth:"100%",maxHeight:"100%",objectFit:"contain"}}):e.jsx(Ue,{className:"no-image-icon",size:40,color:"#e2e8f0"})]})}const Oe=se.button`
