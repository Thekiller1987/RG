...
Esto asumirá que el dinero contado es igual al calculado (Cuadre Perfecto).`))try{const s=new Date().toISOString(),h={userId:((e=t.openedBy)==null?void 0:e.id)||t.usuario_id,closedAt:s,closedBy:{id:999,name:"Admin (Forzado)"},countedAmount:0,notes:"Cierre Forzado por Administrador desde Reportes"};await P.post(`${H}/caja/session/close`,h,{headers:T}),alert("Caja cerrada exitosamente."),S()}catch(s){console.error(s),s.response&&s.response.status===404?(alert("Esta caja ya no existe o ya estaba cerrada. Se actualizará la lista."),S()):alert("Error cerrando caja: "+(((d=(i=s.response)==null?void 0:i.data)==null?void 0:d.message)||s.message))}},M=t=>{var C,_,v;const e=t.stats||q(t),i=window.open("","_blank");if(!i)return;const d=(c==null?void 0:c.empresa_nombre)||"Multirepuestos RG",s=(c==null?void 0:c.empresa_eslogan)||"Repuestos de confianza al mejor precio",h=(c==null?void 0:c.empresa_logo_url)||window.location.origin+"/icons/logo.png",x=`
        ...
        <div class="box">
          ...
          <div class="box-content">
                  ...
                  ${Math.abs((C=e.salidas)==null?void 0:C.reduce((D,y)=>D+Math.abs(y.amount||0),0))>0?`
                      <div class="row"><span>(-) Gastos de Caja (Salidas):</span><span>-${n(Math.abs((_=e.salidas)==null?void 0:_.reduce((D,y)=>D+Math.abs(y.amount||0),0)))}</span></div>
                  `:""}
