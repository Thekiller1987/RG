import{b as oe,r as w,a as P,j as a,X as re,aM as W,aN as ne,W as ie,aO as G,aa as J,af as se,s as g,q as de}from"./vendor-C4uQ3a2a.js";import{a as le}from"./index-DrTQe3aZ.js";import"./scanner-vendor-DfxRpMWJ.js";import"./pdf-vendor-BMvqz6Um.js";const H="/api",ce=`${H}/caja/reporte`,pe=`${H}/caja/abiertas/activas`;function fe(){const o=new Date,b=o.getTimezoneOffset()*6e4;new Date(o.getTime()-b);const d=o.getFullYear(),f=String(o.getMonth()+1).padStart(2,"0"),m=String(o.getDate()).padStart(2,"0");return`${d}-${f}-${m}`}const n=o=>`C$${Number(o||0).toFixed(2)}`,O=o=>{if(!o)return"—";try{const b=new Date(o);return isNaN(b.getTime())?"—":b.toLocaleString("es-NI",{timeZone:"America/Managua",day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0})}catch{return"—"}};function I(o){return o?typeof o=="string"?o:o.name??o.nombre??o.fullName??o.displayName??o.nombre_usuario??o.username??(o.user&&(o.user.name||o.user.username||o.user.displayName))??(o.id?`Usuario ${o.id}`:"—"):"—"}function me(){const[o,b]=w.useState(()=>localStorage.getItem("token")||null);return w.useEffect(()=>{const d=localStorage.getItem("token");d&&d!==o&&b(d)},[o]),o}const ue=de`
...
`;function q(o){let b=[];if(Array.isArray(o==null?void 0:o.transactions))b=o.transactions;else if(o!=null&&o.detalles_json)try{b=(typeof o.detalles_json=="string"?JSON.parse(o.detalles_json):o.detalles_json).transactions||[]}catch(u){console.error("Error parseando snapshot en calculateReportStats",u)}const d=Number((o==null?void 0:o.initialAmount)||(o==null?void 0:o.monto_inicial)||0),f={ventasContado:[],devoluciones:[],cancelaciones:[],entradas:[],salidas:[],abonos:[]};let m=0,j=0,_=0,V=0,A=0,D=0,N=0,R=0,B=0,z=0,k=0,T=0,S=0,L=0;for(const u of b){const t=((u==null?void 0:u.type)||"").toLowerCase();let e=(u==null?void 0:u.pagoDetalles)||{};if(typeof e=="string")try{e=JSON.parse(e)}catch($){console.error("Error parseando pagoDetalles en reporte:",u,$),e={}}(!e||typeof e!="object")&&(e={});let i=Number(e.ingresoCaja!==void 0?e.ingresoCaja:u.amount||0),l=Number(e.totalVenta!==void 0?e.totalVenta:u.amount||0);(t==="salida"||t.includes("devolucion")||t.includes("cancelacion")||t.includes("anulacion"))&&(i=-Math.abs(i),l=-Math.abs(l));const s={...u,pagoDetalles:e,displayAmount:l},h=Number(e.tarjeta||0),x=Number(e.transferencia||0),c=Number(e.credito||0),p=Number(e.efectivo||0)-Number(e.cambio||0),C=Number(e.dolares||0),F=t.startsWith("venta"),v=t.includes("abono")||t.includes("pedido")||t.includes("apartado");if(F){if(e.efectivo!==void 0||e.dolares!==void 0)m+=p,j+=C,_+=p,V+=C;else{const $=i-h-x-c;m+=$,_+=$}A+=h,D+=x,N+=c}else if(v){if(e.efectivo!==void 0||e.dolares!==void 0)m+=p,j+=C,R+=p,B+=C;else{const $=i-h-x-c;m+=$,R+=$}z+=h,k+=x}else t==="entrada"?m+=Math.abs(i):t==="salida"?m-=Math.abs(i):t.includes("devolucion")||t.includes("cancelacion")||t.includes("anulacion")?m+=i:t==="ajuste"?(e.target==="efectivo"&&(m+=i,e.hidden&&(L+=i)),e.target==="dolares"&&(j+=i),e.target==="tarjeta"&&(A+=i),e.target==="credito"&&(N+=i),e.target==="transferencia"&&(D+=i)):m+=i;(t.startsWith("venta")||t.includes("abono")||t==="entrada")&&(S+=Math.abs(l));const E=t==="devolucion"||t.includes("devolucion"),y=t==="cancelacion"||t==="anulacion";t.startsWith("venta")?f.ventasContado.push(s):E?(f.devoluciones.push(s),T+=Math.abs(l)):y?(f.cancelaciones.push(s),T+=Math.abs(l)):t==="entrada"?f.entradas.push(s):t==="salida"?f.salidas.push(s):(t.includes("abono")||t.includes("pedido")||t.includes("apartado"))&&f.abonos.push(s)}const M=Number((o==null?void 0:o.tasaDolar)||36.6);return{cajaInicial:d,netCordobas:m,netDolares:j,movimientoNetoEfectivo:m+j*M,efectivoEsperado:d+m+j*M,efectivoEsperadoCordobas:d+m,efectivoEsperadoDolares:j,ventasContado:f.ventasContado,devoluciones:f.devoluciones,cancelaciones:f.cancelaciones,entradas:f.entradas,salidas:f.salidas,abonos:f.abonos,totalTarjeta:A+z,totalTransferencia:D+k,totalCredito:N,totalNoEfectivo:A+z+D+k+N,vEfectivoC:_,vEfectivoD:V,vTarjeta:A,vTransf:D,vCredito:N,aEfectivoC:R,aEfectivoD:B,aTarjeta:z,aTransf:k,sumDevolucionesCancelaciones:T,totalVentasDia:S,tasaRef:M,totalHidden:L}}const r={primary:"#0f172a",secondary:"#475569",danger:"#dc2626",bg:"#f8fafc",border:"#e2e8f0",text:"#1e293b",textLight:"#64748b"},ge=g.div`
  max-width: 1400px;
  margin: 0 auto;
  padding: 2rem;
...
`,Te=()=>{const o=me(),b=oe(),{settings:d}=le(),[f,m]=w.useState(()=>fe()),[j,_]=w.useState(!1),[V,A]=w.useState([]),[D,N]=w.useState([]),[R,B]=w.useState([]),[z,k]=w.useState(null),T=w.useMemo(()=>{const t={"Content-Type":"application/json"};return o&&(t.Authorization=`Bearer ${o}`),t},[o]),S=w.useCallback(async()=>{var t,e,i;_(!0),k(null);try{const[l,s]=await Promise.all([P.get(ce,{headers:T,params:{date:f}}),P.get(pe,{headers:T})]);A(Array.isArray((t=l.data)==null?void 0:t.abiertas)?l.data.abiertas:[]),N(Array.isArray((e=l.data)==null?void 0:e.cerradas)?l.data.cerradas:[]),B(Array.isArray((i=s.data)==null?void 0:i.abiertas)?s.data.abiertas:[])}catch{k("Error al cargar datos.")}finally{_(!1)}},[T,f]);w.useEffect(()=>{S()},[S]);const L=async t=>{var e,i,l;if(window.confirm(`¿Seguro que deseas FORZAR el cierre de la caja de ${I(t.openedBy)}? 

Esto asumirá que el dinero contado es igual al calculado (Cuadre Perfecto).`))try{const s=new Date().toISOString(),h={userId:((e=t.openedBy)==null?void 0:e.id)||t.usuario_id,closedAt:s,closedBy:{id:999,name:"Admin (Forzado)"},countedAmount:0,notes:"Cierre Forzado por Administrador desde Reportes"};await P.post(`${H}/caja/session/close`,h,{headers:T}),alert("Caja cerrada exitosamente."),S()}catch(s){console.error(s),s.response&&s.response.status===404?(alert("Esta caja ya no existe o ya estaba cerrada. Se actualizará la lista."),S()):alert("Error cerrando caja: "+(((l=(i=s.response)==null?void 0:i.data)==null?void 0:l.message)||s.message))}},M=t=>{var C,F,v;const e=t.stats||q(t),i=window.open("","_blank");if(!i)return;const l=(d==null?void 0:d.empresa_nombre)||"Multirepuestos RG",s=(d==null?void 0:d.empresa_eslogan)||"Repuestos de confianza al mejor precio",h=d!=null&&d.empresa_logo_url?d.empresa_logo_url.startsWith("http")?d.empresa_logo_url:`${"https://sistema.multirepuestosrg.com/api".replace(/\/api$/,"")}${d.empresa_logo_url.startsWith("/")?"":"/"}${d.empresa_logo_url}`:window.location.origin+"/icons/logo.png",x=`
      ...
      <body>
        ...
        <div class="box">
          ...
          <div class="box-content">
                  ...
                  ${Math.abs((C=e.salidas)==null?void 0:C.reduce((E,y)=>E+Math.abs(y.amount||0),0))>0?`
                      <div class="row"><span>(-) Gastos de Caja (Salidas):</span><span>-${n(Math.abs((F=e.salidas)==null?void 0:F.reduce((E,y)=>E+Math.abs(y.amount||0),0)))}</span></div>
                  `:""}
                  <div class="row bold" style="margin-top: 5px;">
                    <span>(=) Flujo Efectivo Neto Hoy:</span>
                    <span>${n(e.vEfectivoC+e.vEfectivoD*e.tasaRef+e.aEfectivoC+e.aEfectivoD*e.tasaRef-e.sumDevolucionesCancelaciones-Math.abs((v=e.salidas)==null?void 0:v.reduce((E,y)=>E+Math.abs(y.amount||0),0)))}</span>
                  </div>
