import{R as Q,r as s,j as e,k as ut,I as bt,aa as jt,D as yt,s as p,v as Ct,x as Se,b2 as wt,af as _e,C as vt,q as Nt,ae as xe,bf as kt,b8 as Ye,g as Qe,bg as lt,bh as $t,J as St,b3 as dt,am as At,Z as Ke,$ as Tt,T as It,aR as Dt,bi as Et,bj as Ft,b4 as ct,ai as zt,an as Rt,bk as Xe,ag as De}from"./vendor-BVroOj2J.js";import{M as Ae,B as S,f as q,a as Pe,I as Te,S as we}from"./POS.styles-SnKkPsfK.js";import{u as Re,$ as Lt,o as Mt,a as _t,r as Pt}from"./index-CbUf7eeN.js";import{A as Ot}from"./AlertModal--TwzGPyw.js";import{r as Vt}from"./searchEngine-BMYcElFi.js";const Bt=p.div`
...
`,ue=t=>`C$${Number(t||0).toFixed(2)}`,rt=t=>t?new Date(t).toLocaleDateString("es-NI",{timeZone:"America/Managua",day:"2-digit",month:"2-digit",year:"numeric"}):"—",Kt=({client:t,onClose:r,onAbonoSuccess:m,showAlert:x})=>{const{addCajaTransaction:w,user:h}=Re(),[d,N]=s.useState(""),[b,L]=s.useState("Efectivo"),[j,l]=s.useState(""),[A,z]=s.useState(!1),[I,i]=s.useState(""),[$,u]=s.useState([]),[M,c]=s.useState(!0),[f,K]=s.useState(null),O=s.useMemo(()=>Number(t==null?void 0:t.saldo_pendiente)||0,[t]);s.useEffect(()=>{const o=async()=>{c(!0);try{const y=localStorage.getItem("token"),V=await Mt(t.id_cliente,y);u(Array.isArray(V)?V:[])}catch(y){console.error("Error cargando tickets:",y),u([])}finally{c(!1)}};t!=null&&t.id_cliente&&o()},[t]);const D=s.useMemo(()=>$.filter(o=>o.saldoRestante>0),[$]),oe=s.useMemo(()=>$.filter(o=>o.saldoRestante<=0),[$]),_=f?Math.min(O,f.saldoRestante):O;s.useEffect(()=>{if(!d){i("");return}const o=parseFloat(d);isNaN(o)||o<=0?i("Ingrese un monto válido mayor a cero."):o>_?i(`El máximo es ${ue(_)}.`):i("")},[d,_]);const re=o=>{o.saldoRestante<=0||(K(y=>(y==null?void 0:y.idVenta)===o.idVenta?null:o),N(""))},v=()=>{N(f?f.saldoRestante.toFixed(2):O.toFixed(2))},ae=s.useCallback((o,y,V,F)=>{const B=Math.max(0,F-o),ie=new Date().toLocaleString("es-NI",{timeZone:"America/Managua"}),T=f?`Venta #${f.idVenta}`:"Cuenta General",fe=`
