...
// Agrega estas funciones en tu api.js
export const fetchTickets = async (token, params = {}) => {
...
export const facturarTicket = async (ticketId, pagoDetalles, token) => {
    const response = await fetch(`${API_BASE}/tickets/${ticketId}/facturar`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify({ pagoDetalles })
    });
    return await response.json();
};

export const cancelarTicket = async (ticketId, token) => {
    const response = await fetch(`${API_BASE}/tickets/${ticketId}/cancelar`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    return await response.json();
...
export const fetchReporteVentas = async (token, params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const response = await fetch(`${API_BASE}/reportes/ventas?${queryString}`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
    return await response.json();
};
