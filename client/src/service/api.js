...
// 1. Si estamos en un entorno de build (ej. Netlify), usamos el prefijo relativo '/api'.
//    Netlify redirigirá '/api/*' a la función /.netlify/functions/server/:splat.
// 2. Si estamos en desarrollo (no hay dominio de Netlify ni URL base definida), usamos localhost.
const isNetlifyBuild = process.env.NODE_ENV === 'production' && window.location.hostname.includes('netlify');
// NOTA: Usamos window.location.hostname para detectar el entorno desplegado.
// Si esta línea no funciona, la alternativa es usar simplemente '/api' como fallback.

// Usamos una definición más simple para garantizar que en Netlify use la ruta relativa '/api'.
// En un entorno de frontend normal, '/api' se refiere a la raíz del sitio (rg11.netlify.app/api)
