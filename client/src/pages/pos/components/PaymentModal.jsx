...
  FaCashRegister, FaHandshake, FaPrint, // 👈 Importamos FaPrint
...
  tasaDolar = 1,
  onClose,
  onFinishSale, // Función que el POS nos pasa
  clientes = [],
  showAlert,
  initialClientId = '0',
}) => {
...
  // ====== FINALIZAR (ASÍNCRONO) - CON BANDERA DE IMPRESIÓN ======
  /**
   * Finaliza la venta, enviando la bandera de impresión al POS.
   * @param {boolean} shouldPrintNow - Indica si se debe intentar imprimir inmediatamente.
   */
...
      credito: credito, // Uso directo de la variable credito calculada
...
      ingresoCaja: Number(ingresoRealEnCaja),
      shouldPrintNow: shouldPrintNow // 👈 ¡Bandera de impresión!
    };

    try {
      const ok = await onFinishSale?.(pagoDetalles); 
      if (ok !== false) {
        onClose?.(); 
      }
    } catch (err) {
...
            {/* Botón final: Dividido en 2 opciones para controlar la impresión */}
            <div style={{ marginTop: 'auto', display: 'flex', gap: '10px' }}>
              <Button
                $cancel
                onClick={() => handleFinish(false)} // 👈 Solo Pagar (shouldPrintNow: false)
                disabled={isButtonDisabled}
                style={{
                  flex: 1, padding: '12px 0',
                  fontSize: '0.95rem',
                  fontWeight: 600,
                  backgroundColor: isButtonDisabled ? '#adb5bd' : '#6c757d',
                  color: 'white',
                  boxShadow: 'none'
                }}
              >
                <FaCheckCircle /> Solo Pagar
              </Button>

              <Button
                pay
                onClick={() => handleFinish(true)} // 👈 Pagar e Imprimir (shouldPrintNow: true)
                disabled={isButtonDisabled}
                style={{
                  flex: 1.5, padding: '12px 0',
                  fontSize: '1.1rem',
                  fontWeight: 800,
                  backgroundColor: isButtonDisabled ? '#adb5bd' : '#28a745',
                }}
              >
                <FaPrint /> Pagar e Imprimir
              </Button>
            </div>
          </div>
        </div>
