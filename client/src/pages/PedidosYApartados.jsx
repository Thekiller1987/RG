...
const handleCreateOrder = async (orderData) => {
        try {
            ...
            // 3. REGISTRAR EL DINERO EN LA CAJA (CORREGIDO)
            const montoAbono = parseFloat(finalOrderData.abonado || 0);

            if (canManageTickets && montoAbono > 0) {
                try {
                    const nuevoId = response?.id || response?.insertId || response?.data?.id || 'NUEVO';

                    // AQUÍ ESTABA EL ERROR: Ahora usamos api.addCajaTx
                    await api.addCajaTx({
                        usuario_id: user.id || user.id_usuario,
                        monto: montoAbono,
                        tipo: 'entrada',
                        descripcion: `Abono Inicial Pedido #${nuevoId} - Cliente: ${finalOrderData.clienteNombre}`,
                        origen: 'PEDIDOS'
                    }, token);
                    
                    console.log("Dinero ingresado a caja correctamente:", montoAbono);
                } catch (errorCaja) {
                    console.error("Error impactando la caja:", errorCaja);
                    showAlert({ 
                        title: "Atención", 
                        message: "El pedido se creó, pero hubo un error registrando el dinero en caja." 
                    });
                }
            }
