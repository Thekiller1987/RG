...
const InventoryManagement = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/products/${productToDelete.id_producto}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchData();
  ...
  const archiveProduct = async (p) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${API_URL}/products/${p.id_producto}/archive`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const executeStockAdjustment = async (product, cantidad, razon) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${API_URL}/products/${product.id_producto}/stock`,
  ...
  const handleAddCategory = async (name) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/categories`, { nombre: name }, { headers: { Authorization: `Bearer ${token}` } });
      await fetchData();
    } catch (error) {
  ...
  const handleAddProvider = async (name) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/providers`, { nombre: name }, { headers: { Authorization: `Bearer ${token}` } });
      await fetchData();
    } catch (error) {
  ...
  const handleDeleteProvider = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/providers/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      await fetchData();
    } catch (error) {
    ...
    <LazyProductImage productId={p.id_producto} productName={p.nombre} onViewFull={(imgSrc) => setViewImage({ isOpen: true, productId: p.id_producto, imageUrl: imgSrc })} />
      ...
      {totalFilteredCount > 0 && (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1.5rem', marginTop: '2rem', marginBottom: '3rem' }}>
          <Button secondary onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>Anterior</Button>
          <div style={{ display: 'flex', gap: '8px' }}>
            {[...Array(totalPages)].map((_, i) => {
              ...
              return (
                <Button key={pageIdx} secondary={pageIdx !== currentPage} primary={pageIdx === currentPage}
                  style={{ minWidth: '40px', padding: '0.5rem' }} onClick={() => setCurrentPage(pageIdx)}>{pageIdx}</Button>
              );
            })}
          </div>
          <Button secondary onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>Siguiente</Button>
        </div>
      )}
      <AnimatePresence>
        {isCreateModalOpen && (
