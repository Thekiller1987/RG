...
      // LLAMADA 1: Obtener todos los usuarios
      const { data } = await axios.get(`${API_BASE_URL}/api/users`, config);
      setUsers(data);
    } catch (err) {
      // Log se mantiene para depuración
...
      const token = localStorage.getItem('token');
      const config = { headers: { Authorization: `Bearer ${token}` } };
      // LLAMADA 2: Eliminar usuario
      await axios.delete(`${API_BASE_URL}/api/users/${userToDelete.id_usuario}`, config);
      setUsers(users.filter(u => u.id_usuario !== userToDelete.id_usuario));
      setIsDeleteModalOpen(false);
      setUserToDelete(null);
    } catch (err) {
      // Mantenemos el error para debugging
...
        }
        // LLAMADA 3: Editar usuario
        await axios.put(`${API_BASE_URL}/api/users/${editingUser.id_usuario}`, updateData, config);
      } else {
        // LLAMADA 4: Registrar nuevo usuario
